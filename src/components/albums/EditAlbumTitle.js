    ...
    return (
        <>
            ...
            <Form onSubmit={handleSubmit}>
                <Form.Group id="title">
                    <Form.Label>Update album title</Form.Label>
                    <Form.Control type="title" onChange={handleTitleChange} placeholder={album.title} value={newAlbumTitle} autoFocus />

                    {album.title && album.title.length < 2 &&
                        <Form.Text className="text__alert">The album title must be at least 2 characters long.</Form.Text>
                    }
                </Form.Group>
                <Button variant="dark" disabled={loading} type="submit">Save</Button>
            </Form>
        </>
    )
}
